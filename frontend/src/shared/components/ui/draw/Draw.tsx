...
export default function Draw({ open, setOpen, content }: { open: boolean; setOpen: (v: boolean) => void; content?: any }) {
    ...
    return (
        <>
            {/* Menu */}
            <div className={`fixed top-0 h-full w-[80%] max-w-xs bg-white shadow-xl z-70 transform transition-transform duration-300 ${slideFromRight(open)} lg:hidden`}>
                ...
                <div className="sticky top-0 h-[50px] flex items-center justify-between px-4 bg-white">
                    ...
                    <button onClick={() => setOpen(false)} className="text-4xl font-bold text-gray-900">
                        ×
                    </button>
                </div>
